...
@section('title', 'Log health record')
...
@section('health-content')
    ...
    @endphp
    @include('modules.partials.header', ['title' => 'Log health record', 'backRoute' => $backRoute])
    @include('modules.partials.flash')

    <form method="POST" action="{{ route('health.records.store') }}" class="dash-farm-form">
        @csrf
        <input type="hidden" name="return_section" value="{{ $returnSection ?? 'overview' }}">
        @include('modules.health._form', ['defaultType' => $defaultType ?? null])
